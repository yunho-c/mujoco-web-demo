...
export async function loadSceneFromURL(mujoco, filename, parent) {
	...
	for (let g = 0; g < model.ngeom; g++) {
		...
		let size = [model.geom_size[g * 3 + 0], model.geom_size[g * 3 + 1], model.geom_size[g * 3 + 2]];
		...
		if (
			...
			material = new THREE.MeshPhysicalMaterial({
				...
				opacity: color[3],
				specularIntensity:
					model.geom_matid[g] != -1 ? model.mat_specular[model.geom_matid[g]] * 0.5 : undefined,
				reflectivity:
					model.geom_matid[g] != -1 ? model.mat_reflectance[model.geom_matid[g]] : undefined,
				roughness:
					model.geom_matid[g] != -1 ? 1.0 - model.mat_shininess[model.geom_matid[g]] : undefined,
