<script lang="ts">
	...
	class MuJoCoDemo {
		...
		gui: GUI | null = null;
		mujocoRoot: MujocoRootGroup | null = null;
		...
		constructor(mujoco: Mujoco) {
			...
			// NOTE: It seems important for a model to be loaded first, before
			//       things like camera, renderer, and animationLoop initialize.
			//       Without this pre-initialization of model, mujoco gets stuck
			//       in a weird, non-interactive state.
			this.model = new mujoco.Model('/working/' + this.params.scene); // HACK
		...
		render(timeMS: number) {
			...
			let numWraps = 0;
			if (this.mujocoRoot && this.mujocoRoot.cylinders && this.mujocoRoot.spheres) {
...
<div bind:this={container} class="aspect-ratio-video h-full w-full"></div>
